...
import { Chat } from "../ContactsPage/components/Chat";
  ...
  return (
      <ChatsPageWrapper>
        ...
        >
          ...
          <Container sx={{ display: "flex", width: "60%", flexWrap: "nowrap" }}>
            <Button sx={{ minWidth: "24px" }}>
              <ChatIcon></ChatIcon>
            </Button>
            <Button sx={{ minWidth: "24px" }}>
        ...
        >
          ...
          {user?.chats.map((item, id) => {
            return <Chat key={id} chatRoomId={item}></Chat>;
          })}
        </Box>
      </ChatsPageWrapper>
  );
